...
def completions_with_backoff(**kwargs):
    ...
    return chater.generator.create(**kwargs)
...
def gpt(prompt, model="llama2", temperature=0.6, max_tokens=1000, n=1, stop=None) -> list:
    ...
    messages = [[{"role": "system", "content": "As professional as possible."},
            {"role": "user", "content": prompt}]]
...
def chatgpt(messages, model="llama2", temperature=0.7, max_tokens=1000, n=1, stop=None) -> list:
    ...
    while n > 0:
        ...
        res = completions_with_backoff(chater=chater, model=model, messages=messages, temperature=temperature, max_tokens=max_tokens, n=cnt, stop=stop)
